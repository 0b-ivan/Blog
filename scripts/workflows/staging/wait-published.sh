...
for attempt in $(seq 1 60); do
  health="$(curl -fsS --max-time 10 -H 'Cache-Control: no-cache'     "${STAGING_URL}/healthz?deploy=${GITHUB_SHA}" 2>/dev/null || true)"
  build_info="$(curl -fsS --max-time 10 -H 'Cache-Control: no-cache'     "${STAGING_URL}/build-info.json?deploy=${GITHUB_SHA}" 2>/dev/null || true)"
  ...
  if [ "$health" = 'ok' ] && [ "$version" = "$EXPECTED_VERSION" ]; then
    page="$(curl -fsS --max-time 10 -H 'Cache-Control: no-cache'       "${STAGING_URL}/?deploy=${GITHUB_SHA}")"
