...
describe('obsidian publisher', () => {
  ...
  it('creates a deletion PR when a published article returns to draft', async () => {
    ...
    publisher.file = vi.fn(async (_filePath, ref) => ref === 'main'
      ? { sha: 'main-file-sha', content: 'published' }
      : { sha: 'branch-file-sha', content: 'published' });
    ...
    expect(publisher.deleteFile).toHaveBeenCalledWith('posts/2026-08-23-test.md', 'obsidian/2026-08-23-test', 'Test');
    expect(publisher.createPullRequest).toHaveBeenCalledWith('obsidian/2026-08-23-test', 'Test', 'posts/2026-08-23-test.md', 'unpublish');
