...
function sizeFigureImage(imageTag) {
  const source = String(imageTag || '');
  const requestedWidth = Number(
    source.match(/\bdata-image-width="(\d{1,3})"/i)?.[1] || 0
  );
  const hasRequestedWidth = Number.isInteger(requestedWidth)
    && requestedWidth >= 10
    && requestedWidth <= 100;
  ...
  const bare = source
    ...
    .replace(/\s+style="[^"]*"/gi, '')
    .replace(/\s+data-image-width="[^"]*"/gi, '')
    .replace(/\s*\/?\s*>$/, '');

  if (hasRequestedWidth) {
    return `${bare} width="${requestedWidth}%" />`;
  }
...
function pdfMetadata(post, options = {}) {
  ...
  const cjkSource = [
    ...
    post.html,
    post.body,
    resolveCoverTitle(post),
    resolveCoverSubtitle(post, { maxLength: 150 })
  ].filter(Boolean).join('\n');
  ...
  return {
    ...
    cjk: containsCjkText(cjkSource) ? 'true' : ''
  };
}
