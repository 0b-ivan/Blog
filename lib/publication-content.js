...
function buildSourcesHtml(entries) {
  ...
  const rows = entries.map((entry, index) => {
    ...
    return `<li id="source-${escapeHtml(entry.id)}"><span class="ebook-source-number">[${index + 1}]</span> ${author}<strong>${escapeHtml(entry.title || entry.id)}</strong>. ${publisher}<a href="${escapeHtml(entry.url || '')}">${escapeHtml(entry.url || '')}</a>.${accessed}${license}</li>`;
  }).join('\n');
