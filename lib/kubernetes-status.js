...
function sanitizedNetworkChaosExperiment(payload) {
  ...
  const experiment = ['network-delay', 'network-loss'].includes(payload.experiment)
    ? payload.experiment
    : '';
  const action = ['delay', 'loss'].includes(payload.action)
    ? payload.action
    : '';
  const source = ['blog', 'search'].includes(payload.source) ? payload.source : '';
  const target = ['blog', 'search'].includes(payload.target) ? payload.target : '';
  if (!experiment || !action || !source || !target) return null;
  ...
  return {
    ...
    source,
    target,
    createdAt: normalizedTimestamp(payload.createdAt),
    completedAt: normalizedTimestamp(payload.completedAt),
    durationMs: boundedNumber(payload.durationMs, 600000),
    latencyMs: boundedNumber(payload.latencyMs, 600000),
    packetLossPercent: Math.min(100, Math.max(0, Number(payload.packetLossPercent) || 0)),
    selected: payload.selected === true,
    allInjected: payload.allInjected === true,
    allRecovered: payload.allRecovered === true,
    failedEvents: boundedNumber(payload.failedEvents, 100),
    passed: payload.passed === true
  };
}
...
function sanitizedKubernetesStatus(payload) {
  ...
  const networkChaosExperiment = sanitizedNetworkChaosExperiment(payload?.lastNetworkChaosExperiment);
  ...
  return {
    ...
    workloads: Array.isArray(payload?.workloads)
      ? payload.workloads.slice(0, 8).map((workload) => ({
        name: ['Blog', 'Search', 'Analytics'].includes(workload?.name) ? workload.name : 'Workload',
    ...
    ...(networkChaosExperiment ? { lastNetworkChaosExperiment: networkChaosExperiment } : {})
  };
}
