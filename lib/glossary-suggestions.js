...
function addSuggestion(target, candidate, location, known, ignored) {
  ...
  const rawKey = normalizeTerm(rawTerm);
  const originalTerm = cleanCandidateTerm(rawTerm);
  const originalKey = normalizeTerm(originalTerm);
  if (
    !originalTerm
    || looksLikeRepositoryPath(originalTerm)
    || ignored.has(rawKey)
    || ignored.has(originalKey)
  ) {
    return;
  }
  ...
  const qualityScore = candidateQualityScore(candidate, term, originalTerm);
  if (
    !key
    || known.has(key)
    || ignored.has(key)
  ...
  const confidence = qualityScore >= 4 ? 'high' : 'medium';
