...
const CONTEXT_STOP_WORDS = new Set([
  'aber', 'als', 'am', 'an', 'auch', 'auf', 'aus', 'bei', 'beziehungsweise', 'betreiben', 'das', 'dem', 'demselben',
  'den', 'der', 'die', 'diesem', 'dieser', 'diesen', 'direkten', 'durch', 'ein', 'eine', 'einem', 'einen', 'einer',
  'eines', 'fest', 'für', 'im', 'in', 'ist', 'lokalen', 'mehreren', 'mit', 'neuen', 'oder', 'ohne', 'über', 'und',
  'via', 'von', 'vor', 'werden', 'wie', 'wird', 'zu', 'zum', 'zur',
...
const GENERIC_COMPOUND_SUFFIXES = new Set([
  'befehle', 'befehlen', 'einstellungen', 'konfiguration', 'manifeste', 'prüfung', 'seite', 'verzeichnisse'
]);

const REPOSITORY_PATH_PREFIXES = new Set([
  'archive', 'assets', 'config', 'docs', 'infra', 'ops', 'posts', 'scripts', 'snippets', 'tests'
]);
...
function looksLikeRepositoryPath(value) {
  ...
  const first = normalizeTerm(normalized.split('/')[0]);
  if (REPOSITORY_PATH_PREFIXES.has(first)) {
    return true;
  }
  ...
  return /\.(?:md|ya?ml|json|js|mjs|cjs|sh|svg|png|jpe?g|webp|css|html?)$/iu.test(normalized);
}
...
function canonicalizeCandidateTerm(value) {
  let term = cleanCandidateTerm(value);
  if (!term) {
    return '';
  }

  const acronymPlural = term.match(/^([\p{Lu}][\p{Lu}\d]{1,})s$/u);
  if (acronymPlural) {
    term = acronymPlural[1];
  }

  const acronymId = term.match(/^([\p{Lu}][\p{Lu}\d]{1,})-IDs?$/u);
  if (acronymId) {
    term = acronymId[1];
  }

  const compound = term.match(/^(.+)-([^\s-]+)$/u);
  if (compound && GENERIC_COMPOUND_SUFFIXES.has(normalizeTerm(compound[2]))) {
    term = cleanCandidateTerm(compound[1]);
  }

  return term;
}

function looksReusableTerm(term) {
  const value = String(term || '').trim();
  if (!value || /^\d/u.test(value) || /^(?:part|teil)-?\d+$/iu.test(value)) {
    return false;
  }
  if (CONTEXT_STOP_WORDS.has(normalizeTerm(value))) {
    return false;
  }

  const classified = classifyToken(value);
  if (classified) {
    return true;
  }

  return /^\p{Lu}[\p{L}\p{N}.+#/_-]*$/u.test(value);
}

function candidateQualityScore(candidate, term, originalTerm = term) {
  let score = candidate.confidence === 'high' ? 2 : 1;

  if (looksReusableTerm(term)) {
    score += 1;
  }
  ...
  if (normalizeTerm(term) !== normalizeTerm(originalTerm)) {
    score += 1;
  }
...
function classifyToken(token) {
  ...
  if (upper.length > 0 && lower.length > 0 && uppercaseAfterFirst && !hasHyphen) {
    return { confidence: 'high', reason: 'Gemischte Groß-/Kleinschreibung wie bei Produkt- oder Projektnamen' };
  }

  const technicalSeparator = (
    /[.+#/_]/u.test(value)
    || (hasHyphen && /\d/u.test(value))
...
function contextualCandidates(line) {
  ...
  for (const leader of CONTEXT_LEADERS) {
    ...
    for (const match of line.matchAll(pattern)) {
      const rawPhrase = String(match[2] || '').trim();
      const phrase = rawPhrase.replace(/[.!?]\s+\p{Lu}[\s\S]*$/u, '');
      if (!phrase) {
        continue;
      }
      ...
      const first = cleanCandidateTerm(parts[0]);
      if (!first || (!classifyToken(first) && !/^\p{Lu}/u.test(first))) {
        continue;
      }
...
function addSuggestion(target, candidate, location, known, ignored) {
  const originalTerm = cleanCandidateTerm(candidate.term);
  const originalKey = normalizeTerm(originalTerm);
  if (!originalTerm || looksLikeRepositoryPath(originalTerm) || ignored.has(originalKey)) {
    return;
  }

  const term = canonicalizeCandidateTerm(originalTerm);
  const key = normalizeTerm(term);
  if (
    !key
    || known.has(key)
    || ignored.has(key)
    || CONTEXT_STOP_WORDS.has(key)
    || candidateQualityScore(candidate, term, originalTerm) < 3
  ) {
    return;
  }

  const confidence = candidateQualityScore(candidate, term, originalTerm) >= 4 ? 'high' : 'medium';
  ...
  if (confidenceRank(confidence) > confidenceRank(entry.confidence)) {
    entry.confidence = confidence;
    entry.term = term;
  }
  entry.reasons.add(candidate.reason);
  if (normalizeTerm(term) !== originalKey) {
    entry.reasons.add(`Normalisiert aus ${originalTerm}`);
  }
