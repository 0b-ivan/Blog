...
const CONTEXT_STOP_WORDS = new Set([
  ...
  'werden', 'wird', 'einen', 'fest', 'ausgeführt', 'beziehungsweise', 'direkten', 'diesen', 'demselben', 'bekannten',
  'ressourcen',
...
const NORMALIZABLE_COMPOUND_SUFFIXES = new Set([
  'Befehl', 'Befehle', 'Befehlen',
  'ID', 'IDs',
...
function canonicalizeCandidateTerm(value) {
  ...
  const acronymPlural = term.match(/^([a-z]?[A-Z]{2,})s$/u);
  if (acronymPlural) {
    term = acronymPlural[1];
  }

  const compound = term.match(/^(.+)-([\p{L}]+)$/u);
  if (
    compound
    && NORMALIZABLE_COMPOUND_SUFFIXES.has(compound[2])
    && /^[\p{Lu}]/u.test(compound[1])
  ) {
    term = compound[1];
  }
...
function looksLikeContextualTerm(value) {
  const first = String(value || '').trim().split(/\s+/u)[0] || '';
  if (!first || CONTEXT_STOP_WORDS.has(normalizeTerm(first))) {
    return false;
  }

  return /^[\p{Lu}]/u.test(first) || /[\d.+#/_-]/u.test(first);
}
...
function classifyToken(token) {
  ...
  if (/^[\p{Lu}]\p{L}*\d+\p{L}*$/u.test(value)) {
    return { confidence: 'high', reason: 'Technischer Produkt- oder Projektname mit Versions-/Ziffernanteil' };
  }
...
function contextualCandidates(line) {
  ...
  for (const leader of CONTEXT_LEADERS) {
    ...
    for (const match of line.matchAll(pattern)) {
      const phrase = String(match[2] || '')
        .replace(/([.!?;:])\s+[\s\S]*$/u, '')
        .trim();
      if (!phrase || !looksLikeContextualTerm(phrase)) {
        continue;
      }
...
function addSuggestion(target, candidate, location, known, ignored) {
  ...
  const term = canonicalizeCandidateTerm(rawTerm);
  const key = normalizeTerm(term);
  if (looksLikeRepositoryPath(rawTerm) || looksLikeRepositoryPath(term)) {
    return;
  }
  if (
    !key
  ...
  if (term !== rawTerm) {
    entry.reasons.add(`Normalisiert: ${rawTerm} → ${term}`);
  }
...
function publicSuggestions(target) {
  return sortSuggestions([...target.values()].map((entry) => {
    const reusable = entry.occurrences.length >= 3;
    return {
      term: entry.term,
      confidence: entry.confidence === 'medium' && reusable ? 'high' : entry.confidence,
      reasons: [
        ...entry.reasons,
        ...(entry.confidence === 'medium' && reusable
          ? ['Mehrfacher technischer Gebrauch erhöht die Wiederverwendbarkeit']
          : [])
      ],
