...
function classifyToken(token) {
  ...
  const uppercaseAfterFirst = characters.slice(1).some((character) => /\p{Lu}/u.test(character));
...
function contextualCandidates(line) {
  const leaders = CONTEXT_LEADERS.map(escapeRegExp).join('|');
  const pattern = new RegExp(
    `\\b(?:${leaders})\\s+([\\p{L}\\p{N}.+#/_-]+(?:\\s+[\\p{Lu}][\\p{L}\\p{N}.+#/_-]+){0,3})`,
    'gu'
  );
  const candidates = [];
...
function addSuggestion(target, candidate, location, known, ignored) {
  const term = String(candidate.term || '').trim().replace(/^[.,;:!?()[\]{}"']+|[.,;:!?()[\]{}"']+$/g, '');
  ...
  if (!existing) {
    target.set(key, {
      ...
      occurrenceKeys: new Set([`${location.file}:${location.line}`])
    });
    return;
  }
...
function mergeSuggestions(groups) {
  ...
  for (const suggestions of groups) {
    for (const suggestion of suggestions) {
      const key = normalizeTerm(suggestion.term);
      ...
      if (!existing) {
        merged.set(key, {
          ...
          occurrenceKeys: new Set((suggestion.occurrences || []).map((item) => `${item.file}:${item.line}`))
        });
        continue;
      }

      if (confidenceRank(suggestion.confidence) > confidenceRank(existing.confidence)) {
      ...
      for (const occurrence of suggestion.occurrences || []) {
        const occurrenceKey = `${occurrence.file}:${occurrence.line}`;
        if (!existing.occurrenceKeys.has(occurrenceKey)) {
          existing.occurrenceKeys.add(occurrenceKey);
