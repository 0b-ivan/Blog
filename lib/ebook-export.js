...
function buildEditorialCoverSvg(post, imageDataUri = '') {
  ...
  <rect width="1600" height="2560" fill="url(#imageFade)"/>
  <rect x="0" y="0" width="1600" height="1240" fill="#ffffff" fill-opacity="0.34"/>`
    ...
    if (absolute.startsWith(`${assetsRoot}${path.sep}`)) {
      try {
        ...
        if (mimeType !== 'image/svg+xml') {
          // Keep the editorial typography on the in-book deckblatt, but use
          // the real raster article image as the EPUB package cover. Apple
          // Books and other readers render this much more reliably in their
          // library views than an SVG package cover.
          return new File(
            [data],
...
function ensureDeckblattItemrefProperties(opf) {
  ...
  return source.replace(itemrefPattern, (_match, rawAttributes) => {
    ...
    const wanted = ['rendition:layout-pre-paginated', 'rendition:spread-none'];
    const propertiesMatch = attributes.match(/\bproperties="([^"]*)"/i);
    ...
    if (propertiesMatch) {
      ...
      attributes = attributes + ' properties="' + wanted.join(' ') + '"';
    }

    return '<itemref ' + attributes + ' />';
  });
}
...
function ensureDeckblattFixedLayout(xhtml) {
  ...
  const viewport = '<meta name="viewport" content="width=1600,height=2560" />';
  const fixedCss = `<style type="text/css">
html, body {
  margin: 0 !important;
  padding: 0 !important;
...
.book-deckblatt--editorial img {
  ...
  if (!patched.includes('object-fit: cover')) {
    patched = patched.replace(/<\/head>/i, fixedCss + '</head>');
  }
