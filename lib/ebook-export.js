...
function buildCoverSvg(post) {
  ...
  <style>
    ...
    .title { font-family: sans-serif; font-size: 128px; font-weight: 800; fill: #111820; }
...
function buildColophon(post, siteUrl) {
  const author = displayAuthor(post.author);
  const sourceUrl = sourceUrlFor(post, siteUrl);
  const coverSource = String(post.coverSourceUrl || post.coverCreditUrl || '').trim();
  const coverLicense = String(post.coverLicense || '').trim();
  const coverLicenseUrl = String(post.coverLicenseUrl || '').trim();
  const coverCredit = String(post.coverCredit || '').trim();
  const coverDetails = coverCredit || coverSource || coverLicense
    ? `<p><strong>Cover:</strong> ${coverCredit ? escapeXml(coverCredit) : 'Pixabay'}${coverSource ? ` · <a href="${escapeXml(coverSource)}">Quelle</a>` : ''}${coverLicense ? ` · ${coverLicenseUrl ? `<a href="${escapeXml(coverLicenseUrl)}">${escapeXml(coverLicense)}</a>` : escapeXml(coverLicense)}` : ''}</p>`
    : '';
...
function buildPhotoCoverSvg(post, imageDataUri) {
  ...
  ${credit ? `<text x="1460" y="2400" class="credit" text-anchor="end">${escapeXml(credit)}</text>` : ''}
  <style>
    ...
    .title { font-family: sans-serif; font-size: 124px; font-weight: 800; fill: #ffffff; }
    ...
    .credit { font-family: sans-serif; font-size: 28px; fill: #ffffff; fill-opacity: 0.75; }
  </style>
</svg>`;
}
