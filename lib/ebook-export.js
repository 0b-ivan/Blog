...
async function createDeckblatt(post, assetRoot, siteUrl) {
  ...
  if (!coverImage.startsWith('/assets/')) {
    if (editorial) {
      ...
      return {
        html: `<style>
html, body { margin: 0 !important; padding: 0 !important; width: 100%; height: 100%; background: #fff; }
.book-deckblatt { margin: 0 !important; padding: 0 !important; width: 100%; height: 100%; overflow: hidden; }
.book-deckblatt svg { display: block; margin: 0; width: 100%; height: 100%; max-width: none; }
</style><div class="book-deckblatt">${svg}</div>`,
        cleanup: async () => {}
      };
    }
